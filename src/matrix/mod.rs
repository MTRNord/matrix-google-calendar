...
pub async fn setup(config: Config<'_>) -> Result<Client, Box<dyn Error>> {
    ...
    if let Some(session) = Session::load(config.session_path.parse().unwrap()) {
        ...
        let login_response = client
            .login(
                ...
                &config.password,
                None,
                Some(&"matrix-calendar-bot".to_string()),
            )
            .await;
    ...
    if crate::MATRIX_CLIENT.set(client.clone()).is_err() {
...
pub async fn start_sync(client: Client, config: Config<'static>) -> Result<(), Box<dyn Error>> {
    client
